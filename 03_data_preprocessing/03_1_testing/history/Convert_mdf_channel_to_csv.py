...
def create_filter(file_name):
    ...
    os.chdir(
        "/home/ase/Dokumente/eh_basics/masterarbeit_eh/01_data_source/mdf_data/Kombination_MdlPwr_MdlBatPwr/test/"
        "txt_files"
    )  # TODO
    s = []
    # print(os.getcwd())
...
def mdf_to_csv(mdf_name, filter):
    ...
    data_filtered.export(
        fmt="csv", filename=name + mdf_name, time_from_zero=False
    )  # sep = ',', decimal = '.',
...
for i in test_filter_dict:  # TODO
    ...
    name = (
        "label_" + str(label_filter_dict.get(i)) + "_"
    )  # string variable input/output
