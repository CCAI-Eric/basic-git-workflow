...
class Split_data:
    ...
    def import_data(self):
        ...
        """
        os.chdir(
            "/home/ase/Dokumente/eh_basics/masterarbeit_eh/02_dataset/current"
        )  # TODO
        self.full_dataset = genfromtxt(
            "Full_concat_dataset_[0]_MdlPwr.csv", delimiter=","
        )
        self.full_dataset_equal = genfromtxt(
            "Full_concat_dataset_[0]_equal_TqEng_MdlPwr.csv", delimiter=","
        )
        self.sindelfingen_dataset = genfromtxt(
            "AitmContsHorzn_MdlPwr300_[0]_Sindelfingen.csv", delimiter=","
        )
        self.gaertringen_dataset = genfromtxt(
            "AitmContsHorzn_MdlPwr300_[0]_Gaertringen.csv", delimiter=","
        )
        self.werk_dataset = genfromtxt(
            "AitmContsHorzn_MdlPwr300_[0]_Werk.csv", delimiter=","
        )
        self.huettlingen_dataset = genfromtxt(
            "AitmContsHorzn_MdlPwr300_[0]_Huettlingen.csv", delimiter=","
        )
    ...
    def choose_data_channels(self):
        ...
        self.ground_truth = np.delete(
            self.ground_truth, 4, 1
        )  # del column 4 --> VectLen
    ...
    def save_dataset(self):
        ...
        np.savetxt(
            "train_ground_truth_full_dataset.csv",
            self.train_ground_truth,
            delimiter=",",
        )
        np.savetxt(
            "test_ground_truth_full_dataset.csv", self.test_ground_truth, delimiter=","
        )
...
if __name__ == "__main__":
    dict_dataset = {
        0: "full_dataset",
        1: "sindelfingen_dataset",
        2: "gaertringen_dataset",
        3: "werk_dataset",
        4: "huettlingen_dataset",
        5: "full_dataset_gleichverteilt_tqeng",
    }
