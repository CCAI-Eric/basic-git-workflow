...
class Preprocessing:
    ...
    def correct_NEng(self):
        os.chdir(
            config["input_path"]
        )  # TODO: Erzeugt eine nan, wodurch das Training nicht funktioniert!
    ...
    def load_csv_data_and_delete_nan(self):
        ...
        SlopFinVal = pd.read_csv(
            config["input_names"]["sample"]["SlopFinVal"], delimiter=","
        )
        ...
        Tm_AmbAirP = pd.read_csv(
            config["input_names"]["sample"]["Tm_AmbAirP"], delimiter=","
        )
        VSlopAFinVectLen = pd.read_csv(
            config["input_names"]["sample"]["VSlopAFinVectLen"], delimiter=","
        )
        Tm_AmbAirTp = pd.read_csv(
            config["input_names"]["sample"]["Tm_AmbAirTp"], delimiter=","
        )
        MVeh = pd.read_csv(config["input_names"]["sample"]["MVeh"], delimiter=",")
        Cod_Diff_Ratio_Calc = pd.read_csv(
            config["input_names"]["sample"]["Cod_Diff_Ratio_Calc"], delimiter=","
        )
        CurbWeight = pd.read_csv(
            config["input_names"]["sample"]["CurbWeight"], delimiter=","
        )
        WhlPA_Circumfer = pd.read_csv(
            config["input_names"]["sample"]["WhlPA_Circumfer"], delimiter=","
        )
    ...
    def create_coordinates_ti(self, horzn_len):
        ...
        coordinates_array = np.array(
            coordinates_array
        )  # TODO: Überprüfe die Shape 3521877
    ...
    def concat_data(self):
        ...
        sample = pd.concat(
            [
                ...
                self.VFinVal,
                self.VSlopAFinPosn,
                self.VSlopAFinVectLen,
                self.Cod_Diff_Ratio_Calc,
                self.CurbWeight,
                self.Tm_AmbAirP,
                self.Tm_AmbAirTp,
                self.WhlPA_Circumfer,
            ],
            axis=1,
        )
        print(sample.head())
        ...
        self.sample = data[
            :, :13
        ]  # TODO: Achte darauf, dass die coordinaten Spalten mitgenommen werden und erst später aussortieren!
        self.label = data[:, 13:]
        print(sample.shape)
        print(label.shape)
    ...
    def data_scaler(self):
        ...
        scaler = MinMaxScaler(
            feature_range=(
                config["scaler_feature_range"]["min"],
                config["scaler_feature_range"]["max"],
            )
        )
        ...
        scaler_filename = (
            "scaler"
            + config["split_data_names"]["api"]
            + config["split_data_names"]["train_samples"]
        ...
        scaler_filename = (
            "scaler"
            + config["split_data_names"]["api"]
            + config["split_data_names"]["train_ground_truth"]
            + config["scaler_format"]
        )
        joblib.dump(scaler, scaler_filename)
    ...
    def save_data(self):
        ...
        np.savetxt(
            config["split_data_names"]["train_samples"]
            + config["split_data_names"]["data_format"],
            self.train_samples,
            delimiter=",",
        )
        np.savetxt(
        ...
        np.savetxt(
            config["split_data_names"]["test_samples"]
            + config["split_data_names"]["data_format"],
            self.test_samples,
            delimiter=",",
        )
        np.savetxt(
            config["split_data_names"]["test_ground_truth"]
            + config["split_data_names"]["data_format"],
            self.test_ground_truth,
            delimiter=",",
        )
