...
class Preprocessing:
    ...
    def load_csv_data(self):
        ...
        for num, name in enumerate(dataset_list):
            ...
            sample_array = np.ones(
                shape=(dataset_len_list[num], 1)
            )  # TODO: Wie sollen die Zeilen ausgelesen werden?
            ...
            for label in label_list:
                ...
                for i, d in enumerate(horizon_len):  # +1
                    ...
                    row[
                        ...
                        np.nan
                    )  # TODO: targets werden nicht durch horizon_len beschränkt
                    rows.append(row)
                rows = np.array(rows)
                rows = rows[np.logical_not(np.isnan(rows))]
                rows = rows.reshape(rows.shape[0], 1)
            ...
            self.samples_gesamtdatensatz = np.append(
                self.samples_gesamtdatensatz, self.sample_array, axis=0
            )
            self.targets_gesamtdatensatz = np.append(
                self.targets_gesamtdatensatz, self.target_array, axis=0
            )
    ...
    def save_data(self):
        ...
        print("Saving data")
        dataset.to_csv(
            self.save_name + config["savedataformat"], header=False, index=False
        )
