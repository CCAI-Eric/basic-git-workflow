...
# Open config file
with open("03_data_preprocessing/03_5_preprocess_MdlPwr_BatPwr_kombi/03_5_Var_2_Zeitschritt/config_preprocess.yml",
          "r") as stream:
...
class concat_data:
    ...
    def lade_Gesamtdatensatz(self):
        ...
        self.array = pd.read_csv(os.path.join(path, dataset_name))  # , usecols=col_list)
    ...
    def load_datasets(self):
        ...
        for dataset in dataset_list:
            print(dataset)
            dataset_arr = np.array(pd.read_csv(os.path.join(path, dataset), delimiter=','))
    ...
    def balance_dataset(self):
        ...
        X_over, y_over = undersample.fit_resample(X=self.samples,
                                                  y=self.targets)  # Todo: Problem: Input contains NaN, infinity or a value too large for dtype('float64')
    ...
    def analyze_min_max_values_for_normalization(self):
        ...
        for i in range(0, 1400, 200):
            ...
            w_nans = numpy.isnan(target_array)  # filter die nans raus, vermasseln die Min- und Max Analyse
    ...
    def analyze_data_channels(self):
        ...
        plt.xlabel(channel)
        plt.ylabel("Anzahl (-)")
        plt.title("Häufigkeitsverteilung " + channel + " für den Gesamtdatensatz - alle Ereignisse [i]", size=20)
        ser = pd.Series(arr)
        print(ser.describe())
    ...
    def save_dataset(self):
        ...
        path_gesamtdatensatz = config["output_path_gesamtdatensatz"].split("/Variante_2")[0]
        ...
        final_df.to_csv(config["output_path_gesamtdatensatz"] + "/" + self.name + config["savedataformat"],
