...
"""
# Open config file
with open("03_data_preprocessing/03_5_preprocess_MdlPwr_BatPwr_kombi/03_5_Var_2_Zeitschritt/config_preprocess.yml", "r") as stream:
    config = yaml.safe_load(stream)

def normalization_func(X_feature_max, X_feature_min, X_max, X_min, X):
    X_norm = ((X_feature_max - X_feature_min) / (X_max - X_min)) * (X - X_min) + X_feature_min
    return X_norm
...
class prepare_data:
    ...
    def split_data(self):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.samples, self.targets, random_state=config["seed"],
                                                            test_size=1-train_ratio, shuffle=config["shuffle"])
        self.X_val, self.X_test, self.y_val, self.y_test = train_test_split(self.X_test, self.y_test, random_state=config["seed"],
                                                            test_size=test_ratio/(test_ratio+validation_ratio), shuffle=False)
        print("Train size: 80 %")
    ...
    def normalize_data(self):
        ...
        self.targets.iloc[:, i:j] = normalization_func(X_feature_max, X_feature_min, X_max, X_min, self.targets.iloc[:, i:j])
        print(self.targets.iloc[:, i:j].describe())
        ...
        print(self.targets.iloc[:, i:j].describe())
        self.targets.iloc[:, i:j] = normalization_func(X_feature_max, X_feature_min, X_max, X_min,
                                                         self.targets.iloc[:, i:j])
        print(self.targets.iloc[:, i:j].describe())
        ...
        self.samples.iloc[:, i] = normalization_func(X_feature_max, X_feature_min, X_max, X_min,
                                                       self.samples.iloc[:, i])
        print(self.samples.iloc[:, i].describe())
        ...
        self.samples.iloc[:, i:j] = normalization_func(X_feature_max, X_feature_min, X_max, X_min,
                                                       self.samples.iloc[:, i:j])
        print(self.samples.iloc[:, i:j].describe())
    ...
    def save_dataset(self):
        ...
        print("Saving data")
        if config["save_splitted_dataset"]:
            self.X_train.to_csv(config["X_train"] + config["savedataformat"], header=False, index=False)
            self.X_val.to_csv(config["X_val"] + config["savedataformat"], header=False, index=False)
            self.X_test.to_csv(config["X_test"] + config["savedataformat"], header=False, index=False)
            self.y_train.to_csv(config["y_train"] + config["savedataformat"], header=False, index=False)
            self.y_val.to_csv(config["y_val"] + config["savedataformat"], header=False, index=False)
            self.y_test.to_csv(config["y_test"] + config["savedataformat"], header=False, index=False)
        else:
            ...
            self.targets.to_csv(config["targets"] + config["savedataformat"], header=False, index=False)
            self.samples.to_csv(config["samples"] + config["savedataformat"], header=False, index=False)
